...
Qlik SaaS offers several authentication methods through the qlik-api-ts library:
...
@Component({
  selector: 'app-qlik-integration',
  templateUrl: './qlik-integration.component.html',
  styleUrl: './qlik-integration.component.css'
})
export class QlikIntegrationComponent implements OnInit {
  
  ngOnInit() {
    this.setupQlikAuthentication();
  }

  async setupQlikAuthentication() {
    const hostConfig = {
      authType: "apikey",
      host: "my-org.region.qlikcloud.com", // your Qlik SaaS tenant
      apiKey: "",
...
export class JwtAuthComponent implements OnInit {
  ...
  fetchJwtToken(): Promise {
    // Replace with your actual backend endpoint
    return this.http.get('/api/get-qlik-jwt').toPromise()
...
export class QapTicketAuthComponent implements OnInit {
  ...
  fetchTicket(): Promise {
    // Replace with your actual backend endpoint
    return this.http.get('/api/get-qlik-ticket').toPromise()
...
[25] https://help.qlik.com/en-US/sense-admin/November2022/pdf/Deploy%20Qlik%20Sense%20Enterprise%20on%20Windows.pdf
